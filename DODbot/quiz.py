    ...
    if selected_quiz:
        ...
        for time, info in quiz_schedule.items():
            ...
            if time > current_time:
                ...
                break
        if upcoming:
            n_quiz_time, (n_quiz_name, n_location,
                          n_quiz_function_name) = upcoming
        else:
            n_quiz_time, (n_quiz_name, n_location, n_quiz_function_name) = min(
                quiz_schedule.keys())
        bot.send_message(
            m.chat.id, f"Ближайший квиз начнется в {n_quiz_time} в {n_location}")
        return
        ...
        if message_text in words:
            if selected_quiz:
                ...
                if quiz_func:
                    ...
                    bot.send_message(
                        message.chat.id, "Ошибка: функция квиза не найдена.")
            else:
...
def quiz1(message):
    ...
    bot.send_message(
        message.chat.id, "Вопрос 1: Какой из этих городов является столицей Франции?")

    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("1) Берлин",
                             callback_data=f"select_station:1:{user}"),
        InlineKeyboardButton("2) Париж",
                             callback_data=f"select_station:2:{user}"),
        InlineKeyboardButton("3) Рим",
                             callback_data=f"select_station:3:{user}"),
        InlineKeyboardButton("4) Лондон",
                             callback_data=f"select_station:4:{user}")
    )
...
def check_answer_1(call):
    ...
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("1) Атлантический",
                             callback_data=f"select_ocean:1:{user}"),
        InlineKeyboardButton("2) Индийский",
                             callback_data=f"select_ocean:2:{user}"),
        InlineKeyboardButton("3) Тихий",
                             callback_data=f"select_ocean:3:{user}"),
        InlineKeyboardButton("4) Южный",
                             callback_data=f"select_ocean:4:{user}")
    )
...
def check_answer_2(call):
    ...
    markup.add(
        ...
        InlineKeyboardButton("3) Наверное",
                             callback_data=f"select_sos:3:{user}"),
        InlineKeyboardButton("4) Не хочу отвечать",
                             callback_data=f"select_sos:4:{user}")
    )
    bot.send_message(chat_id, "Выберите ответ:", reply_markup=markup)
...
def check_answer_3(call):
    ...
    bot.send_message(
        chat_id, f"Квиз пройден. Сумма баллов: {check_quiz_points(user)}", reply_markup=main_keyboard())
...
def quiz3(message):
    ...
    bot.send_message(
        message.chat.id, "Вопрос 1: Как называется фильм, в котором персонаж по имени Ганди спасает мир?")
    bot.send_message(
        message.chat.id, "А) Гладиатор\nБ) Мстители\nВ) Терминатор\nГ) Индийский фильм 'Ганди'")
...
def check_answer_7(message):
    if message.text.lower() == "г":
        ...
        bot.send_message(
            message.chat.id, "Неправильно, правильный ответ: Г) Индийский фильм 'Ганди'")

    bot.send_message(
        message.chat.id, "Вопрос 2: Какой фильм получил Оскар за лучший фильм в 1994 году?")
    bot.send_message(
        message.chat.id, "А) Форрест Гамп\nБ) Побег из Шоушенка\nВ) Парк Юрского периода\nГ) Легенда о Тарзане")
...
def check_answer_8(message):
    if message.text.lower() == "а":
        ...
        bot.send_message(
            message.chat.id, "Неправильно, правильный ответ: А) Форрест Гамп")

    bot.send_message(
        message.chat.id, "Вопрос 3: Какая актриса сыграла роль Джульетты в фильме 'Ромео и Джульетта' (1996)?")
    bot.send_message(
        message.chat.id, "А) Меган Фокс\nБ) Скарлетт Йоханссон\nВ) Клэр Дейнс\nГ) Дженнифер Лоуренс")
...
def check_answer_9(message):
    if message.text.lower() == "в":
        ...
        bot.send_message(
            message.chat.id, "Неправильно, правильный ответ: В) Клэр Дейнс")
...
def quiz4(message):
    ...
    bot.send_message(
        message.chat.id, "Вопрос 1: Кто является автором песни 'Bohemian Rhapsody'?")
    bot.send_message(
        message.chat.id, "А) Элтон Джон\nБ) Фредди Меркьюри\nВ) Лед Зеппелин\nГ) Пол Маккартни")
...
def check_answer_10(message):
    if message.text.lower() == "б":
        ...
        bot.send_message(
            message.chat.id, "Неправильно, правильный ответ: Б) Фредди Меркьюри")

    bot.send_message(
        message.chat.id, "Вопрос 2: Какой музыкальный инструмент играет на концертах группы Metallica?")
    bot.send_message(
        message.chat.id, "А) Гитара\nБ) Фортепиано\nВ) Тромбон\nГ) Ударные")
...
def check_answer_11(message):
    if message.text.lower() == "а":
        ...
        bot.send_message(
            message.chat.id, "Неправильно, правильный ответ: А) Гитара")

    bot.send_message(
        message.chat.id, "Вопрос 3: Как называется дебютный альбом группы The Beatles?")
    bot.send_message(
        message.chat.id, "А) Abbey Road\nБ) Let It Be\nВ) Please Please Me\nГ) Revolver")
...
def check_answer_12(message):
    if message.text.lower() == "в":
        ...
        bot.send_message(
            message.chat.id, "Неправильно, правильный ответ: В) Please Please Me")
...
def quiz5(message):
    ...
    bot.send_message(
        message.chat.id, "Вопрос 1: Что из перечисленного является операционной системой?")
    bot.send_message(
        message.chat.id, "А) Windows\nБ) Google\nВ) Samsung\nГ) Mozilla")
...
def check_answer_13(message):
    if message.text.lower() == "а":
        ...
        bot.send_message(
            message.chat.id, "Неправильно, правильный ответ: А) Windows")

    bot.send_message(
        message.chat.id, "Вопрос 2: Как называется самая популярная социальная сеть?")
    bot.send_message(
        message.chat.id, "А) Twitter\nБ) Instagram\nВ) Facebook\nГ) LinkedIn")
...
def check_answer_14(message):
    if message.text.lower() == "в":
        ...
        bot.send_message(
            message.chat.id, "Неправильно, правильный ответ: В) Facebook")

    bot.send_message(
        message.chat.id, "Вопрос 3: Как называется язык программирования, который был создан для написания сценариев?")
    bot.send_message(
        message.chat.id, "А) Python\nБ) Ruby\nВ) Java\nГ) JavaScript")
...
def check_answer_15(message):
    if message.text.lower() == "а":
        ...
        bot.send_message(
            message.chat.id, "Неправильно, правильный ответ: А) Python")
